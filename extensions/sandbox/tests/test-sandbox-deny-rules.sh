...
# Set the sandbox profile path for the launcher
export SANDBOX_PROFILE="$(cd .. && pwd)/profiles/claude-code-sandbox.sb"
...
if WORKING_DIR="/tmp" DEV_WORKSPACE="/tmp" ../launcher.sh ls "$HOME/.ssh" 2>&1 | grep -q "Operation not permitted"; then
...
if WORKING_DIR="/tmp" DEV_WORKSPACE="/tmp" ../launcher.sh ls "$HOME/.aws" 2>&1 | grep -q "Operation not permitted"; then
    echo -e "${GREEN}✓ PASS${NC} - Access correctly denied"
else
    echo -e "${RED}✗ FAIL${NC} - Access was NOT denied (security risk!)"
fi
...
echo ""
echo "Test 3: .claude directory access:"
echo "Command: ls \$HOME/.claude"
OUTPUT=$(WORKING_DIR="/tmp" DEV_WORKSPACE="/tmp" ../launcher.sh ls "$HOME/.claude" 2>&1 || true)
...
echo ""
echo "Test 4: .agent-os directory access:"
echo "Command: ls \$HOME/.agent-os"
OUTPUT=$(WORKING_DIR="/tmp" DEV_WORKSPACE="/tmp" ../launcher.sh ls "$HOME/.agent-os" 2>&1 || true)
if echo "$OUTPUT" | grep -q "Operation not permitted"; then
    echo -e "${RED}✗ FAIL${NC} - Access was denied (should be allowed)"
else
    echo -e "${GREEN}✓ PASS${NC} - Access correctly allowed"
...
echo ""
echo "Test 5: DEV_WORKSPACE (/tmp) access:"
echo "Command: ls /tmp"
OUTPUT=$(WORKING_DIR="/tmp" DEV_WORKSPACE="/tmp" ../launcher.sh ls "/tmp" 2>&1 || true)
if echo "$OUTPUT" | grep -q "Operation not permitted"; then
    echo -e "${RED}✗ FAIL${NC} - Access was denied (should be allowed)"
else
    echo -e "${GREEN}✓ PASS${NC} - Access correctly allowed"
...
echo ""
echo "Test 6: Current working directory write access:"
echo "Command: touch /tmp/test_write_$$.txt"
if WORKING_DIR="/tmp" DEV_WORKSPACE="/tmp" ../launcher.sh touch "/tmp/test_write_$$.txt" 2>&1 >/dev/null; then
...
echo "  - Tests 1-2: Should show '✓ PASS - Access correctly denied'"
echo "  - Tests 3-6: Should show '✓ PASS - Access correctly allowed'"
echo ""
