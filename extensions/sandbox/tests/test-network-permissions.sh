...
run_test() {
    ...
    if [ "$DRY_RUN_ONLY" = "true" ]; then
        ...
        full_command="../launcher.sh --dry-run bash -c \"$test_command\" 2>&1 | grep -q 'Would execute'"
    else
        # Actually run in sandbox
        full_command="WORKING_DIR=\"/tmp\" DEV_WORKSPACE=\"/tmp\" ../launcher.sh bash -c \"$test_command\" 2>/dev/null"
    fi
...
test_port_with_server() {
    ...
    if WORKING_DIR="/tmp" DEV_WORKSPACE="/tmp" ../launcher.sh \
